...
`gate` is the whole proof, and **it covers all three lanes** — core, the
MCP host and the MAUI host:
...
**Two of those three suites used to run nowhere but a workflow**, and every
...
the GitHub Release with the `.vsix` and one sparky archive per OS
attached. It also records a deployment for `sparky`, which is what puts
it in the repo home's Deployments sidebar beside Pages.

The unix archives are cut on the Linux runner deliberately: the execute
bit only survives a tar made on a filesystem that has one, and a binary
