# Shoddy Sound — Implementation Prompt

## The name

A West Riding mill town ran on sound. The working day was called and dismissed
by the mill **buzzer** — the steam hooter on the engine house roof, heard
across the whole valley — and inside the shed the din of the looms was so
...
builtin words themselves keep their BASIC lineage — `SOUND` and `PLAY` are
what GW-BASIC called them, and Shoddy honours its ancestors — just as the
...
it — `mill run` just keeps the resulting assembly in memory.
...
src/
  ...
  Shoddy.Tests/
machines/             — stdlib: seq, str, matrix, dict, stats, file, isam, simplex,
...
- **Known collision, found in advance:** `machines/stats.shoddy` exports
  `Def Freq(xs)`. The buzzer machine must not define `Freq` — use `MidiFreq`
...
five builtins that cover the three ways a program can own a note's lifetime.
No new value type, no new thread of our own, no window required — a console
program may beep.
...
- **No new `VType`.** Sound needs no handle: effects are anonymous and
  channels are numbers, chip-style. (WAV samples would need a handle — that
...
- **Sound never blocks and never fails the program.** Every sound word is
  ...
  fine. Argument errors (bad channel, negative duration) still raise — those
  are program bugs.
- **Silence is a no-op, not an error.** Headless (woven output run via
  ...
  precedent — presentation degrades quietly — and deliberately *not* the
...
1. **Fire-and-forget** — `SOUND freq ms`. The *runtime* owns the lifetime.
   ...
   error). Game effects. A chord is three consecutive calls — they start
   within microseconds and OpenAL mixes them.

2. **Key-state** — `NOTEON ch freq` / `NOTEOFF ch`. The *program* owns the
   ...
   program's own choice — no stealing heuristics.

3. **Sequenced** — `SOUNDQUEUE ch freq ms`. The *score* owns the timing.
   ...
   **sample-accurately** via OpenAL buffer queueing. Do not drive melodies
   ...
   drops missed ticks by design — fine for animation, audibly lumpy for
...
coexist. **Eight channels, numbered 1–8** (Shoddy is 1-based everywhere).
...
```csharp
// Shoddy.Runtime — Buzzer.cs. No Silk.NET, nothing but BCL.
...
public static class BuzzerRegistry
{
    ...
    // running (woven output via dotnet, headless tests). All five are
    // called from the Shoddy thread — OpenAL has no thread affinity, so
...
`?.Invoke` — null means silence. There is no handle, no queue, no semaphore:
...
Five words, added to both `Engine.Builtin()` and `Engine.BuiltinWords`.
All five are **zero-result, like `PRINT` and `SLEEP`** — there is no handle
...
NOTEON      ( ch freq -- )
    ...
    already sounding a held note: retune in place (no re-attack) — this is
    what a keyboard glide or pitch-bend wants, and it is implemented as an
    AL_PITCH ratio, not a new buffer. On a channel with a queue: the queue
    is flushed first (deterministic replace).

NOTEOFF     ( ch -- )
    Release channel ch's held note. A channel with nothing held: no-op.
    Releases ramp gain over ~5 ms before stopping — a hard stop clicks.
...
SOUNDQUEUE  ( ch freq ms -- )
    ...
    is a REST of ms milliseconds — a score needs silence it can count on.
...
- `ch` outside 1–8: `"NoteOn: channel must be 1..8, got 12"`.
- `freq <= 0` where a pitch is required (`SOUND`, `NOTEON`; `SOUNDQUEUE`
  allows exactly 0 as a rest). Clamp nothing — a wrong octave is easier to
  find when 0.5 Hz raises than when it plays inaudibly.
- `ms < 0`.
- A per-channel queue cap (1024 pending notes). Exceeding it raises with a
  message naming the fix: feed long scores incrementally from Tick events.
  Blocking would violate "sound never blocks"; dropping notes would corrupt
  the music; loud is right.

Frequencies are Hz and fractional (`Number` all the way down) — equal
...
native package (`Silk.NET.OpenAL.Soft.Native.*` for the target platforms —
...
- **Init failure → permanent silence.** No audio device (CI, a server, a
  container) must not crash or error a program. Failed init sets a flag and
  every delegate becomes a no-op — identical behaviour to headless, which is
  the point.
...
- **Teardown drains, then stops.** At mill exit (the same place
  `ScribblerWindows` tears down): held `NOTEON` notes stop immediately —
  they would never end — but queued notes are allowed to finish, polled at
...
- 44100 Hz, 16-bit, mono, square wave. Amplitude around 0.2 of full scale —
...
- **Cache synthesized buffers keyed by `(freq, ms)`**, bounded (a few
  hundred entries, evict oldest). The same jump-blip every frame must cost
  one synthesis, not sixty per second.

### Sources

- **Effect pool:** ~16 sources. Allocation sweeps for `Stopped` sources to
  recycle; none free → steal the source with the oldest start (track a
  running sequence number). Never block, never error.
- **Channels:** 8 dedicated streaming sources. `SOUNDQUEUE` first unqueues
  any processed buffers (this is also when cache-owned buffers are released
  back), then queues the new one. **The classic streaming bug:** a source
...
- `NOTEOFF`'s release ramp is a short loop on the calling thread stepping
  the source gain down over ~5 ms, then stop, then restore gain. Five
  milliseconds on the Shoddy thread is imperceptible and buys click-free
  releases without an envelope engine.
...
above). In every case each sound word validates its arguments — bad
arguments raise even in silence, so headless tests exercise real code — and
...
describes — do not foreclose it, do not do it now.)
...
forever. Sound has no wait, so a silent no-op is safe — and it means a
...
**Do not define `Freq`** — `stats.shoddy` already exports it and both fold
...
Def MidiFreq(midi As Number) As Number
    Rem 440 * 2 ^ ((midi - 69) / 12) — A4 = 69 = 440 Hz, equal temperament
...
Def MmlNotes(mml As String) As List Of BuzzerNote
    Rem PURE — parse MML to a note list, no I/O. This split is what makes
...
- `A`–`G` — a note; optional `#`/`+` (sharp) or `-` (flat); optional length
  number (`4` = quarter note, `8` = eighth…); optional trailing `.`
  (dotted, ×1.5).
- `On` — octave, 0–8, default 4 (scientific: O4 contains middle C —
  document this, GW-BASIC's octaves sat one lower).
- `<` / `>` — octave down / up.
- `Ln` — default length, default 4.
- `Tn` — tempo in quarter-notes per minute, default 120.
- `Pn` / `Rn` — a rest of length n (dotted allowed) → `FreqHz` 0.
...
a startup jingle (sequenced), a mouse glissando (key-state — the mouse
...
Def EventLoop(sc As Scribbler, held As Bool) As Scribbler
...
that every `Select Case` branch yields the same arity — the quit branches
...
GLFW auto-repeat `KeyDown`s by carrying a held-key set in its loop state —
...
- Install fake `BuzzerRegistry` delegates that record their arguments;
  assert `SOUND`/`NOTEON`/`SOUNDQUEUE`/`NOTEOFF`/`SOUNDSTOP` pass through
  exactly what the program said, in order.
- With all delegates null, every sound word is a silent no-op — no throw.
...
plays a buffer" — synthesis math (period rounding, envelope shape, cache
...
- [ ] `BuzzerRegistry` in `Shoddy.Runtime/Buzzer.cs` — five nullable
      delegates, no Silk.NET, no new `VType`, nothing waits on sound
- [ ] 5 builtins (`SOUND`, `NOTEON`, `NOTEOFF`, `SOUNDQUEUE`, `SOUNDSTOP`)
      in `Engine.Builtin()` **and** `Engine.BuiltinWords`; all zero-result
- [ ] Channels 1–8; range, sign, and queue-cap validation raises even when
      delegates are null; freq 0 legal only as a `SOUNDQUEUE` rest
- [ ] No folded-name collisions: the five builtin words checked against
      every machine's `Def`/`Type`/field names; **no `Freq` anywhere** —
      `stats.shoddy` owns it; fields are `FreqHz`/`Ms`
- [ ] `Shoddy.Mill.csproj` references `Silk.NET.OpenAL` + OpenAL Soft
      natives; `Shoddy.Runtime.csproj` unchanged
- [ ] Device opens lazily on first sound call, from the Shoddy thread; init
      failure degrades to permanent silent no-op, never an error
- [ ] Effect pool (~16) recycles stopped sources and steals the oldest;
      never blocks, never errors
- [ ] Held notes loop whole periods; retune is `AL_PITCH`, not a new
      buffer; releases ramp ~5 ms; fixed buffers bake attack/release ramps
- [ ] Streaming channels unqueue processed buffers on each call and restart
      a source that ran dry; buffer cache keyed `(freq, ms)` and bounded
- [ ] Mill exit drains queued notes (capped), stops held notes immediately;
      `mill dap` has working sound; `mill lex`/`weave`/`machine` and every
      non-sound program unaffected, including startup cost
- [ ] `machines/buzzer.shoddy`: `MidiFreq`, `NoteMidi`, `NoteFreq`,
...
- [ ] Headless tests added and passing; no test asserts on audio output
