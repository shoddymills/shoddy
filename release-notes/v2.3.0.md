...
- **[Fettler](https://shoddymills.github.io/shoddy/fettler.html) --glob file,
  ...
  an assistant actually carries out on a source tree --glob find, search, read,
  write, edit, replace, move, copy, delete, extract, and run a declared task --glob
...
- **A boundary you declare, and nothing outside it exists.**
  A path outside every declared tree is refused with exit 4 --glob *identically
...
- **Seven permissions, per tree and per folder.** `list read create update
  ...
  *replaces* what its tree grants rather than adding to it, so it can take a
  permission away --glob which is what makes "may reorganise, may never destroy"
...
- **The tool cannot write the files that say what it may do.** `.fettler.json`
  ...
  long --glob write a task declaration, then run what you just wrote.
...
- **A write that would introduce a credential is refused** --glob outcome
  ...
  keep and a rule people switch off. Two tiers --glob issued credentials whose
  ...
  nothing else. **It is a safety net, not a boundary** --glob base64 the value or
...
- **`fettle doctor`** answers, in one screen, whether Fettler is registered
  ...
  boundary --glob a pre-approved `sed -i`, an allow on a built-in editor, an
...
- **It reads more than source.** Notebooks as cells, PDFs as text per page,
  images as facts plus a native image part, and archives as a manifest --glob
...
- **The gate covers all four lanes, not one of them.** `scripts/shoddy.ps1
  ...
  --glob so a change outside their paths triggered none of them.
...
- **A receipt names the tree it was earned against, not the commit.**
  ...
  from before the morning's work. `publish` was never at risk --glob it demands a
  clean tree and a receipt for the exact commit it will tag --glob but a maintainer
...
- **Debug and Release stopped sharing one mill's `bin/`.** Every
  ...
  weave-beside-then-rename protects a compiler *reading* the previous weave --glob
...
- **The Fettler page claimed an atomicity the code does not promise.** A batch
  resolves all-or-nothing --glob every file read, hash-checked and every
  substitution located before the first byte moves --glob but the *writing* itself
...
- **`mills/pac-vt100/mill.manifest` had lost `julian`.** A generated file
  ...
  that noticed was the MAUI host lane's build --glob which nothing local ran and
