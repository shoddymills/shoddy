...
- **`fettle setup` now denies the shell as well as the built-in file tools.**
  ...
  A shell is a complete route round the boundary, and not a second-order one --glob
  ...
  **This stops your build and test commands the first time you run it.** That
  ...
  Allow back only what the project needs --glob `"allow": ["Bash(git status:*)"]` --glob
...
- **`doctor` check 2.8 covers all eight names.** A project with the six file
  tools denied and the shell wide open used to report a clean bill of health --glob
...
- **`doctor` check B.17 exempts a narrowed shell allow.** `allow: Bash(git
  ...
  B.17 matched on the tool name --glob so it reported the recommended configuration
...
- **`fettle setup` and `fettle doctor` could not run before `.fettler.json`
  existed --glob and `setup --local` is what creates it.** Every verb resolved the
...
- **`fettle setup --hooks` installed nothing when the deny list was already
  ...
  nothing to deny --glob so asking for it on an already-configured project, or at
...
- **[Quick Start](https://shoddymills.github.io/shoddy/fettler-quickstart.html)** --glob
