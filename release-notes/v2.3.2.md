...
without a restart --glob a permission you revoke is gone on the very next
request.
...
- **Editing `.fettler.json` or `.fettler.local.json` did nothing to a
  ...
  mid-session did not appear until the client was restarted --glob and, the
  ...
  does --glob **both files are re-read before every request** --glob so an edit
  ...
  every request, naming the fault, until it is fixed --glob never falling
